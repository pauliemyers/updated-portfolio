...
function draw() {
  ...
  for (var i = 0; i < howwide; i++) {
    for (var j = 0; j < howtall; j++) {
      ...
      else // center pixel is DEAD
      {
        ...
        if(neighbors == 0 || neighbors == 3) result = 1; else result = 0;
        
        //while this does not necessarily simulate "life", it does create a self-sustaining image
        //to show this sustain in a way that is non-damaging, I created the frameCount bit below
      }
     // write pixels into destination image, scaled to 0 or 255:
      img[1-whichimage].set(i, j, color((result+random(1))*255), color(result*255));
    }
  }
  if(frameCount % 15 == 0) {
  ...
  //the above bit of code draws the new image, but it only updates every 15 frames.
  //this allows for the illusion of motion without hurting the viewers' eyes
}
